...
| **v.max\_size()** | Returns the maximum size that the vector can hold. | O(1) |
...
| **v.push\_back()** | Add an element to the end. | O(1) |
| **v.pop\_back()** | Remove the last element. | O(1) |
...
| **replace(v.begin(),v.end(),value,replace\_value)** | Replace all the value with replace\_value. Not under a vector. | O(N) |
